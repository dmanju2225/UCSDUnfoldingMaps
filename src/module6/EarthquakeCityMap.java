...
public class EarthquakeCityMap extends PApplet {
	...
	// add
	// code tqo modify is countryQuakes, where you will store the number of
	// earthquakes
	// per country.
	...
	public void draw() {
		 ...
		 */
		/*
		 * EarthquakeMarker[] toArray()
		 * {
		 *  EarthquakeMarker[] arr = new EarthquakeMarker[this.length];
		 *  int i =0;
		 *  for(marker : markers)
		 *  {
		 *    arr[i] = marker;
	...
	private void sortAndPrint(int numToPrint) {
		...
		for (int i = 0; i < n - 1; i++) {
			int minIndex = i;
			for (int j = i + 1; j < n; j++) {
				if (((EarthquakeMarker) arr[j]).compareTo((EarthquakeMarker)arr[minIndex]) < 0) {
					minIndex = j;
				}
			}
			swap(arr, i, minIndex);
		...
		if (numToPrint <= n) {
			for (int i = 0; i < numToPrint; i++) {
				System.out.println("count is " + i + " " + ((EarthquakeMarker) arr[i]).getMagnitude());
			}
		} else {
			for (int i = 0; i < n; i++) {
				System.out.println("count is " + i + " " + ((EarthquakeMarker) arr[i]).getMagnitude());
