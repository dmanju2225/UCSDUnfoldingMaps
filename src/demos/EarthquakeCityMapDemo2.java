...
//	    map = new UnfoldingMap(this, 200, 50, 650, 600, new MBTilesMapProvider(mbTilesString));
	    ...
	    map.zoomToLevel(1);//zoom in
	    MapUtils.createDefaultEventDispatcher(this, map);	

	    PointFeature valdiviaEq = new PointFeature(new Location(-38.14f,-73.03f));//creating pointfeature object which takes location object as parameter
	    valdiviaEq.addProperty("title", "Valdivia, Chile");//adding properties for pointfeatute object which stores single location
	    ...
	    List<PointFeature> bigEarthquakes = new ArrayList<PointFeature>();//creating list which contains pointfeatute objects
	    bigEarthquakes.add(valdiviaEq);//adding each pointfeature(locaation with properties) to list
...
//	    
	    List<Marker> markers = new ArrayList<Marker>();//creating empty list of Markers
	    for (PointFeature eq: bigEarthquakes) {
	    	SimplePointMarker sm=new SimplePointMarker(eq.getLocation(), eq.getProperties());//creating marker for each point feature
	    	//sm.setColor(255);//setting color for each ,marker
	    	markers.add(sm);//adding marker to list of markers	
	    }
	    int yellow = color(255, 255, 0);//color method returns an integer
	    int gray = color(150,150,150);
	    
	    for (Marker mk :markers) {//iterating over list of Marker
	    	if ( (int) mk.getProperty("year") > 2000 ) {//for each Marker,getting property and it returns an object so type casting to int
	    		mk.setColor(yellow);//for each marker setting color..setcolor() method takes int as parameter
	    	}
	    	else {
	    		mk.setColor(gray);
	    	}
	    }
	    map.addMarkers(markers);//adding markers to the map
	    /*Location valLoc = new Location(-38.14f,-73.03f);
        Marker val = new SimplePointMarker(valLoc);
	    map.addMarker(val);*/
	}
	public void draw() {
		background(255, 204, 0);//from processing library,PApplet class, red,green,blue..
	   // background(10);
	    map.draw();
	   // addKey();
	}
